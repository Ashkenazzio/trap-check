...
def format_examples_for_prompt(examples: dict) -> str:
    ...
    lines.append("---")
    lines.append("## Scoring Guide (start at 25, require STRONG evidence to increase)")
    lines.append("- 70-100: 🚨 TOURIST TRAP - MULTIPLE corroborated exploitation patterns")
    lines.append("- 50-69: ⚠️ MIXED - Requires multiple independent trap signals")
    lines.append("- 0-49: ✓ NOT A TRAP - Default for venues without clear trap patterns")
    lines.append("")
    lines.append("**ONLY increase score for:** MULTIPLE 'tourist trap' warnings, specific fake review accusations, price+quality complaints together")
    lines.append("**DO NOT increase for:** service issues, crowds, waits, location, high prices alone, ONE external opinion, tourist languages in reviews")
    lines.append("")
